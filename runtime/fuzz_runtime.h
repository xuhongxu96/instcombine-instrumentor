...
template <typename T>
T *record_stacktrace_with_loc(T *val, const char *file, int line,
                              const char *func) {
  if (val)
    record_stacktrace((void *)val, file, line, func);
  return val;
}
} // namespace llvm_fuzz

#define __llvm_fuzz_record(val)                                                \
  ::llvm_fuzz::record_stacktrace_with_loc((val), __FILE__, __LINE__,           \
                                          __PRETTY_FUNCTION__)
...
#define __llvm_fuzz_call(expr)                                                 \
  __extension__({                                                              \
    ::llvm_fuzz::CallScope __llvm_fuzz_cs(__FILE__, __LINE__,                  \
                                          __PRETTY_FUNCTION__);                \
    (expr);                                                                    \
  })
