...
- `.github/workflows/wasm-verify.yml` — **verification-only**. Runs on push/PR (paths gated to wasm-relevant files) and `workflow_dispatch`. Builds the wasm bundle via `build_wasm.sh`, runs `wasm/test/smoke_wasm.mjs`, uploads a 14-day `wasm-bundle-latest` workflow artifact so PR authors can preview a build without publishing. No GitHub Release attachment. No tag triggers.
- `.github/workflows/wasm-publish.yml` — the wasm publishing workflow. Mostly a thin orchestrator over the per-step scripts under `.github/scripts/wasm-publish/`. Modes selected via `workflow_dispatch` input (`mode: weekly-stable | daily-main | specific-ref`) or by cron schedule (Mon 05 UTC = `weekly-stable`, daily 06 UTC = `daily-main`). The workflow now splits into a `resolve` job, an unlocked `build` job, and a mutexed `finalize` job: `dispatch.sh` normalizes the event into a mode + inputs tuple → `resolve.sh` (wrapping `resolve_refs.sh`) emits a `<dirname>\t<llvm_commit>` table of refs to build and uploads it as an artifact → `build_one.sh` is invoked per ref to reset/clone/patch/build/smoke-test and copy outputs into a staging directory uploaded as a workflow artifact → `finalize` downloads both artifacts, runs `setup_worktree.sh` to check out the `wasm-pkgs` branch (auto-initializing the orphan branch on first run), then `finalize.sh` copies the staged outputs into the branch, prunes old `main-*` snapshots (daily-main only, keeps last `prune_main` = 7), regenerates `wasm-pkgs/manifest.json` via `build_manifest.mjs`, and commits + pushes. `build/llvm-wasm` is intentionally kept across iterations so CMake's incremental rebuild and ccache reuse stay valid across refs. Only the finalize job is serialized (`group: wasm-pkgs`), so overlapping runs can build in parallel but still commit atomically. The build/finalize job names include the current resolved build list. Permissions: `contents: write` only. Workflow inputs let you `force_rebuild` (skip the "already published" short-circuit) and `dry_run` (build without pushing).
- `.github/workflows/wasm-pages.yml` — runs on push to `main` (web paths), PRs (build only), `workflow_dispatch`, and Mon 09 UTC cron. Runs `web/scripts/build-manifest.mjs` against the live `wasm-pkgs/manifest.json` to produce the same-origin manifest the webapp loads at startup. `workflow_dispatch` exposes `bundle_mode` (`remote` / `hybrid` / `bundled`, default `hybrid`), `bundle_count` (default 5), `include_commit_count` (default 0), and `must_bundle` (CSV, merged with `wasm-must-bundle.txt`). The build also bakes `VITE_REMOTE_MANIFEST_URL` so the webapp can fall back to the wasm-pkgs raw URL if the same-origin manifest is ever missing.
...
**Helper scripts are grouped by owning workflow under `.github/scripts/`:** `shared/` (`ccache_keys.sh`, `install_toolchain.sh`, `resolve_llvm_commit.sh`) is used by multiple workflows. Everything else lives under the subdirectory named after its sole consumer — `native-release-auto/`, `native-release-manual/`, `wasm-publish/`, `wasm-pages/`. Inside each subdir filenames drop the redundant workflow prefix (so the wasm-publish dispatch step lives at `wasm-publish/dispatch.sh`, not `wasm-publish/wasm_publish_dispatch.sh`).
...
`native-weekly-canary.yml` passes `--branch main` because its `Select LLVM ref` step rewrites `llvm_commit.txt` with whatever upstream ref the dispatcher pointed at (often LLVM main tip) — that ref isn't a stable cache identity, so weekly's writes pin to the `branch-main` bucket and rely on the broader fallbacks for restore hits. `wasm-publish.yml` similarly passes `--branch wasm-pkgs` because it cycles through multiple LLVM refs in a single run, so a per-ref key doesn't make sense — restore-keys fall back via `branch-main-` to whatever weekly/main last wrote. `native-build.yml` and `wasm-verify.yml` leave the source detection to `llvm_commit.txt`.
