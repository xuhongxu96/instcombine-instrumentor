...
- `web/src/wasm/manifest.ts` + `web/scripts/build-manifest.mjs` — runtime manifest type and a Node-only builder. At Pages-deploy time the builder hits `GET /repos/{owner}/{repo}/releases`, filters to releases whose assets include both `instcombine_driver.js` and `instcombine_driver.wasm`, downloads the newest 10 stable releases into `web/public/wasm/<sanitized-tag>/`, and writes `web/public/wasm/manifest.json` listing every qualifying release (bundled or on-demand). Pre-releases and older stable releases land in the manifest as `bundled: false` entries served from the GitHub asset URL. Releases missing either wasm asset are filtered out.
...
- `.github/workflows/pages.yml` — runs on push to `main` (web paths), PRs (build only), `release: { types: [released] }`, and `workflow_dispatch`. Downloads the latest `wasm.yml` artifact into `web/public/wasm/_latest/` as a synthetic "(latest build)" bootstrap entry, runs `web/scripts/build-manifest.mjs --bootstrap` to populate `web/public/wasm/manifest.json` + `release_*/` directories from GitHub Releases, then builds and deploys the SPA to GitHub Pages.
