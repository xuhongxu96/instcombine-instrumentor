...
- `web/src/wasm/manifest.ts` + `web/scripts/build-manifest.mjs` — runtime manifest type and a Node-only builder. At Pages-deploy time the builder hits `GET /repos/{owner}/{repo}/releases`, filters to releases whose assets include both `instcombine_driver.js` and `instcombine_driver.wasm`, then sorts the survivors newest-first by parsing the `llvmorg-X.Y.Z[-rcN]` semver out of each tag (stable beats its own rc of the same X.Y.Z; tags that don't match the pattern sink to the bottom and order by publish date). It downloads at most one stable release per LLVM major version (the highest minor.patch wins) into `web/public/wasm/<sanitized-tag>/`, capped at `--bundle-count` entries (default 10), and writes `web/public/wasm/manifest.json` listing every qualifying release (bundled or on-demand). Older minor.patch in an already-bundled major, pre-releases, and anything beyond the cap land in the manifest as `bundled: false` entries served from the GitHub asset URL. Releases missing either wasm asset are filtered out. `manifest.defaultTag` is the first bundled non-prerelease entry — i.e. the highest stable LLVM version.
