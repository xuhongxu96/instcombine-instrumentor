...
**ccache caching.** All three build workflows (`build.yml`, `wasm.yml`, `weekly-llvm.yml`) share the same actions/cache@v4 strategy. Cache keys come from `.github/scripts/compute_ccache_keys.sh <workflow> <os>` (run in a `Compute ccache keys` step just before the cache restore) and follow the layout `ccache-<workflow>-<os>-<kind>-<id>-<run-id>` where `<kind>-<id>` is derived from the triggering git ref, **not** the LLVM commit SHA. Keying on SHA means every LLVM bump throws away the cache; branch/tag keying lets successive runs against the same ref share state and lets new release tags inherit from main. The four kinds:

- `branch-<name>` — push / PR / `workflow_dispatch` on a branch. PRs use `GITHUB_HEAD_REF`; slashes sanitize to `_`.
- `tag-llvmorg-<X>.<Y>.<Z>[-rcN]` — `release/llvmorg-X.Y.Z[-rcN]` tags.
- `tag-commit-<YY>-<MM>-<DD>` — `release/<YYMMDD>-<12hex>` commit-snapshot tags.
- `tag-other-<sanitized>` — any other `release/*` tag.

`restore-keys` walks from the most-specific same-id prefix outward — for llvmorg tags that's `X.Y.Z` → `X.Y.` → `X.` → any `tag-llvmorg-`; for commit snapshots `YY-MM-DD` → `YY-MM-` → `YY-` → any `tag-commit-` — then falls back to the `branch-main-` bucket so a brand-new release inherits main's cache, then to the workflow-and-os catch-all. Non-main branch builds (PRs, feature branches) also fall back to `branch-main-`. The per-run id suffix forces a fresh write each run (`actions/cache@v4` only writes when the primary misses), so the LRU 10 GB budget can evict stale older entries and we never get stuck on a cache locked in by the first successful run.

`wasm.yml` additionally pins `EMSDK_VERSION` and exports `CCACHE_COMPILERCHECK=string:emsdk-<version>` around `build_wasm.sh` so emcc (a Python wrapper) gets a stable compiler hash; `build_wasm.sh` / `build_patched_llvm.sh` set `LLVM_CCACHE_BUILD=ON` and `CCACHE_BASEDIR=$(realpath .)` so paths are relativized for cross-runner portability. Each workflow runs `ccache -z` after restore and `ccache -s -v` after the build so the per-run hit/miss numbers in the logs reflect only that run.
