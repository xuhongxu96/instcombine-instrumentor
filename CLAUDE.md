...
**ccache caching.** All three build workflows (`build.yml`, `wasm.yml`, `weekly-llvm.yml`) share the same actions/cache@v4 strategy. Cache keys come from `.github/scripts/compute_ccache_keys.sh <workflow> <os>` (run in a `Compute ccache keys` step just before the cache restore) and follow the layout `ccache-v2-<workflow>-<os>-<kind>-<id>-<run-id>` where `<kind>-<id>` is derived from **`llvm_commit.txt`** — the actual source-of-truth for which LLVM is being built, *not* the triggering git ref. Deriving from `llvm_commit.txt` means every workflow targeting the same LLVM version shares a bucket: PRs, push-to-main, and the release tag for the same `llvmorg-X.Y.Z` all hit each other's cache, and keying lives or dies with the LLVM bump rather than each git ref. (The `-v2-` prefix invalidates older SHA-keyed caches; bump it again if the layout changes.) The three kinds the helper emits:
...
`weekly-llvm.yml` passes `--branch main` because its `Select LLVM ref` step rewrites `llvm_commit.txt` with whatever upstream ref the dispatcher pointed at (often LLVM main tip) — that ref isn't a stable cache identity, so weekly's writes pin to the `branch-main` bucket and rely on the broader fallbacks for restore hits. The other two workflows leave the source detection to `llvm_commit.txt`.
