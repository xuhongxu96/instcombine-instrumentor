...
- `.github/workflows/wasm-publish.yml` — the wasm publishing workflow. Mostly a thin orchestrator over the per-step scripts under `.github/scripts/wasm-publish/`. Modes selected via `workflow_dispatch` input (`mode: weekly-stable | daily-main | specific-ref`) or by cron schedule (Mon 05 UTC = `weekly-stable`, daily 06 UTC = `daily-main`). `dispatch.sh` normalizes the event into a mode + inputs tuple → `resolve.sh` (wrapping `resolve_refs.sh`) emits a `<dirname>\t<llvm_commit>` table of refs to build → `setup_worktree.sh` checks out the `wasm-pkgs` branch (auto-initializes the orphan branch on first run) → `build_one.sh` is invoked per ref to reset/clone/patch/build/smoke-test and copy outputs into `wasm-pkgs-branch/<dirname>/` → `finalize.sh` prunes old `main-*` snapshots (daily-main only, keeps last `prune_main` = 7), regenerates `wasm-pkgs/manifest.json` via `build_manifest.mjs`, and commits + pushes. `build/llvm-wasm` is intentionally kept across iterations so CMake's incremental rebuild and ccache reuse stay valid across refs. Single commit + push per run; concurrency-grouped globally (`group: wasm-pkgs`) so daily/weekly/manual runs queue cleanly. No artifact upload (outputs land on the branch). Permissions: `contents: write` only. Workflow inputs let you `force_rebuild` (skip the "already published" short-circuit) and `dry_run` (build without pushing).
